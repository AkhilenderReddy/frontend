...
function Address({ setShowAddress, data, setData = "", type = "" }) {
  ...
  //   googleMapsApiKey: process.env.REACT_APP_URL_GOOGLE_API_KEY,// here i will be using google maps API
  //   libraries: ["places"]
  // });
              ...
              <div className=" md:my-8  my-4 md:w-[50rem] w-full relative ">
                {!isLoaded ? (
                  ...
                  <>
                    ...
                    {/* current location */}
                    <div
                      className="absolute bottom-8 right-4  text-purple-600 h-8 w-8 rounded-full hover:bg-purple-500 hover:text-white items-center flex justify-center"
                      onClick={() => {
                        handleLocation();
                      }}
                    >
                      {currentlocationLoading ? (
                        <svg
                          aria-hidden="true"
                          role="status"
                          className=" w-6 h-6  text-purple animate-spin"
                          viewBox="0 0 100 101"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
