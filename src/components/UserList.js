...
function UserList({ setCurrentUser, search, setSearch, role, userUrl }) {
  ...
  return (
    <>
      <header className="   border-b-4   p-4  shadow-xl bg-purple-50  dark:bg-gray-800  border-purple-300  dark:border-gray-500   sticky top-0 left-0 ">
        <div className="   text-xl md:text-2xl  font-semibold  text-purple-500   dark:text-white   flex  md:gap-0 gap-2 md:flex-row flex-col  md:items-center md:justify-between   ">
          <h1 className="first-letter:uppercase">{role === "AGENT" ? "NGO's" :role.toLowerCase() + "s"}</h1>
          <Search
            setSearch={setSearch}
            search={search}
